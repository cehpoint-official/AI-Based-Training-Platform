...
const Profile = () => {
    ...
    const handleSubmitApiKey = async (event) => {
        ...
        setProcessing(true);
        const uid = sessionStorage.getItem('uid');
        const postURL = serverURL + '/api/profile'; // Assuming the same URL for API key
        try {
                ...
                <div className='flex-1 flex items-center justify-center py-10 flex-col'>
                    <div className="md:w-2/5 w-4/5 m-auto py-4 no-scrollbar ">
                        ...
                        <div className='py-6'>
                            ...
                            <Button onClick={() => { setShowPasswordForm(false); setShowApiKeyForm(true); }} className='mb-4 w-full dark:bg-white dark:text-black bg-black text-white font-bold rounded-none'>Add API Key</Button>
                            ...
                            {showApiKeyForm && (
                                <form onSubmit={handleSubmitApiKey}>
                                    <div className='mb-6'>
                                        <div className="mb-2 block">
                                            <Label className="font-bold text-black dark:text-white" htmlFor="apiKey" value="API Key" />
                                        </div>
